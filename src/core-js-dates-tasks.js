...
function getNextFriday(date) {
  ...
  const tillFriday =
    (12 - currentWeekDay) % 7 === 0 ? 7 : (12 - currentWeekDay) % 7;
  date.setDate(date.getDate() + tillFriday);
  return date;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.round((new Date(dateEnd) - new Date(dateStart)) / oneDay) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return currDate - startDate >= 0 && endDate - currDate >= 0;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysInMonth; i += 1) {
    ...
    if ([0, 6].includes(currDate.getDay())) {
      count += 1;
    }
  }
  return count;
...
function getWeekNumberByDate(date) {
  ...
  firstMonday.setDate(
    jan4.getDate() - (jan4.getDay() === 0 ? 6 : jan4.getDay() - 1)
  );

  const diffInDays = Math.floor((date - firstMonday) / (24 * 60 * 60 * 1000));
...
function getNextFridayThe13th(date) {
  ...
  for (let j = currYear; j < currYear + 2; j += 1) {
    for (let i = date.getMonth(); i < 12; i += 1) {
      const thisThirteenth = new Date(j, i, 13);
      if (thisThirteenth.getDay() === 5) {
        return thisThirteenth;
      }
    }
  }
  return null;
...
function isLeapYear(date) {
  ...
  return (
    dateYear % 4 === 0 &&
    (dateYear % 100 !== 0 || (dateYear % 100 === 0 && dateYear % 400 === 0))
  );
}
